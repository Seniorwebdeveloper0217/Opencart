...
class ModelKulerCommon extends Model
{
	...
	public function mapProductDisplayOptions(array $settings) {
		if (!empty($settings['show_product_name'])) {
			...
			$settings['deal_date']          = !empty($settings['show_product_deal_date']) ? $settings['show_product_deal_date'] : 0;
			...
			$settings['show_product_deal_date']         = !empty($settings['deal_date']) ? $settings['deal_date'] : 0;
			$settings['show_product_image']             = !empty($settings['image']) ? $settings['image'] : 0;
	...
	public function loadProductTemplate(array $setting, array $product, $type)
	{
		...
		{
			return $this->load->view($this->config->get('config_template') . "/template/common/_{$type}_product.tpl", $data);
		}
		else
		{
			return $this->load->view("default/template/common/_{$type}_product.tpl", $data);
		}
	}
